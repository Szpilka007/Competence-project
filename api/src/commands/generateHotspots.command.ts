...
export class GenerateHotspotsCommand {
  ...
  phi(distance: number, angle: number): number {
    return Math.asin(
      Math.sin(CENTER_LATITUDE) * Math.cos(distance / EARTH_RADIUS) +
        Math.cos(CENTER_LATITUDE) * Math.sin(distance / EARTH_RADIUS) * Math.cos(angle)
    );
  }
  ...
  lambda(distance: number, angle: number, latitude: number): number {
    return (
      CENTER_LONGITUDE +
      Math.atan2(
        Math.sin(angle) * Math.sin(distance / EARTH_RADIUS) * Math.cos(CENTER_LATITUDE),
